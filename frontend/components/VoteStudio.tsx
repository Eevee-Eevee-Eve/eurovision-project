  ...
  return (
    ...
    >
      <div className="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3">
        <span className="show-chip px-2 py-1 text-[10px] font-semibold text-white/88">#{option.value}</span>
        <div className="min-w-0">
          {option.countryName ? (
            <ArtistCountryBadge countryName={option.countryName} flagUrl={option.flagUrl} compact />
          ) : (
            <span className="show-chip px-2 py-1 text-[10px] text-arenaMuted">{option.caption}</span>
          )}
          <p className="mt-1 line-clamp-1 text-[12px] leading-5 text-arenaMuted">
            {option.artist ? `${option.artist}${option.song ? " • " + option.song : ""}` : option.caption}
          </p>
        </div>
        <span className="inline-flex h-6 w-6 items-center justify-center rounded-full border border-white/10 bg-white/[0.04] text-arenaBeam">
          {selected ? <Check size={14} /> : <span className="h-1.5 w-1.5 rounded-full bg-white/25" />}
        </span>
      </div>
    </button>
  );
        ...
        <button type="button" onClick={onOpen} className="flex min-w-0 items-center gap-3 text-left">
          ...
          <div className="min-w-0">
            <div className="mt-0.5 flex min-w-0 items-center gap-1.5">
              <ArtistCountryBadge countryName={countryName} flagUrl={flagUrl} className="max-w-full flex-1" />
              {noteBadge ? (
                <span
                  className="pointer-events-none inline-flex h-[1.1rem] w-[1.1rem] shrink-0 items-center justify-center rounded-full border border-white/12 bg-arenaBeam/10 text-arenaBeam"
                  title={noteBadge}
                  aria-label={noteBadge}
                >
...
export function VoteStudio({ roomSlug, stageKey }: { roomSlug: string; stageKey: StageKey }) {
  ...
  const text = useMemo(() => (
    language === "ru"
      ? {
          ...
          title: "Голосование",
          description:
            "Собери свой личный порядок: перетаскивай артистов, открывай карточки, оставляй заметки и отправляй итог.",
          stageForming: "Состав этапа ещё формируется",
          stageFormingText: (count: number, total: number) => `Сейчас в списке ${count} из ${total} участников. Когда состав будет полным, голосование обновится автоматически.`,
          roomLabel: "Комната",
          accountReady: "Аккаунт активен",
          accountNeeded: "Чтобы сохранить порядок, нужен аккаунт",
          loginHint: "Расставлять артистов можно сразу. Чтобы отправить итог, войди в аккаунт на главной.",
          loginAction: "На главную",
          lockedState: "Этап закрыт",
          searchPlaceholder: "Поиск по артисту, стране или песне",
          mobileSearchPlaceholder: "Поиск: артист или страна",
          searchDragHint: "Сейчас включён поиск. Очисти строку, чтобы снова удобно перетаскивать артистов.",
          aboutArtist: "Об исполнителе",
          noteLabel: "Моя заметка",
          noteHint: "Запиши короткую мысль по номеру, чтобы не забыть впечатление к финальному выбору.",
          ...
          officialProfileHint: "Открой официальную карточку артиста в новом окне.",
          choosePlace: "Поставить артиста на место",
          choosePlacePlaceholder: "Выбрать место",
          moveHigher: "Выше",
          moveLower: "Ниже",
          watchVideo: "Посмотреть видео",
          watchVideoHint: "Видео поможет быстро вспомнить номер перед перестановкой.",
          resetRanking: "Сбросить расстановку",
          resetRankingConfirmTitle: "Сбросить расстановку?",
          ...
          resetRankingConfirmAction: "Сбросить расстановку",
          ...
          submitDisabledLocked: "Этап уже закрыт",
          submitDisabledUnchanged: "Сначала переставь хотя бы одного артиста",
          submitSuccess: "Ответ сохранён. Пока этап открыт, можно отправить новую версию.",
          loadError: "Не удалось загрузить голосование для этого этапа.",
          saveOrderHint: "Порядок и заметки сохраняются на этом устройстве. В зачёт идёт последняя отправленная версия.",
          resubmitHint: "Твой выбор уже отправлен. Пока этап открыт, можно переставить артистов и отправить обновление.",
          countdownClosingSoon: "Отправка скоро закроется",
          countdownGlobal: "До закрытия отправки",
          countdownGlobalText: "Когда таймер закончится, отправить новый выбор уже не получится.",
          countdownPersonal: "Для тебя открыто ещё",
          countdownPersonalText: "Организатор дал тебе немного дополнительного времени.",
          countdownWarning: "Меньше минуты до закрытия",
          blockedMessage: "Организатор временно закрыл тебе отправку ответа в этой комнате. Смотреть артистов всё ещё можно.",
        ...
        : {
          ...
          title: "Voting",
          description:
            "Build your personal order: drag acts, open cards, add notes, and submit your final pick.",
          stageForming: "This stage lineup is still forming",
          stageFormingText: (count: number, total: number) => `${count} of ${total} acts are listed right now. The voting screen updates automatically when the lineup is complete.`,
          roomLabel: "Room",
          accountReady: "Account active",
          accountNeeded: "You need an account to save your order",
          loginHint: "You can rank acts right away. Sign in on the homepage when you are ready to submit.",
          loginAction: "Open homepage",
          lockedState: "Stage closed",
          searchPlaceholder: "Search by act, country, or song",
          mobileSearchPlaceholder: "Search: artist or country",
          searchDragHint: "Search is active. Clear the field to make drag-and-drop comfortable again.",
          aboutArtist: "About the artist",
          noteLabel: "My note",
          noteHint: "Save a quick thought about the act so the final order is easier to finish.",
          ...
          officialProfileHint: "Open the official artist profile in a new tab.",
          choosePlace: "Place this artist at",
          choosePlacePlaceholder: "Choose place",
          moveHigher: "Move up",
          moveLower: "Move down",
          watchVideo: "Watch video",
          watchVideoHint: "Use the video to quickly remember the act before reordering.",
          resetRanking: "Reset placement",
          resetRankingConfirmTitle: "Reset placement?",
          ...
          resetRankingConfirmAction: "Reset placement",
          ...
          submitDisabledLocked: "Stage is already closed",
          submitDisabledUnchanged: "Move at least one act first",
          submitSuccess: "Answer saved. While the stage is open, you can submit an updated version.",
          loadError: "Unable to load voting for this stage.",
          saveOrderHint: "Your order and notes stay on this device. The latest submitted version is the one that counts.",
          resubmitHint: "Your latest pick is already submitted. While the stage is open, you can reorder and submit again.",
          countdownClosingSoon: "Submission is closing soon",
          countdownGlobal: "Submission closes in",
          countdownGlobalText: "When the timer reaches zero, new submissions close.",
          countdownPersonal: "Open for you for",
          countdownPersonalText: "The organizer gave you a little extra time.",
          countdownWarning: "Less than a minute left",
          blockedMessage: "The organizer temporarily closed submissions for you in this room. You can still browse acts.",
  ...
  const globalStageOpen = room?.predictionWindows[stageKey] ?? false;
  ...
  useEffect(() => {
    ...
    socket.on("toggle", (payload: { predictionWindows: Record<StageKey, boolean>; submissionCountdowns?: RoomDetails["submissionCountdowns"]; showState?: RoomDetails["showState"] }) => {
      setRoom((current) => {
        if (!current) return current;
        return {
          ...current,
          predictionWindows: payload.predictionWindows || current.predictionWindows,
          submissionCountdowns: payload.submissionCountdowns || current.submissionCountdowns,
          showState: payload.showState || current.showState,
        };
      });
      if (account) {
        void fetchMyPrediction(roomSlug, stageKey)
          ...
          .catch(() => {
            setSubmissionOpen(Boolean(payload.predictionWindows?.[stageKey]));
            setSubmissionOverrideEndsAt(null);
          });
      } else {
        setSubmissionOpen(Boolean(payload.predictionWindows?.[stageKey]));
  ...
  const countdownRemainingMs = activeCountdownEndsAt
  ...
  const canDrag = currentStageOpen && deferredQuery.trim().length === 0;
      ...
      : !currentStageOpen
        ...
        : !lineupReady
          ...
          : !hasCustomRanking
            ...
            : null;
  const stageClosed = !currentStageOpen;
  const submitHelperText = submitDisabledReason
    || (currentStageOpen && hasSubmittedPrediction ? text.resubmitHint : text.saveOrderHint);
  const showCountdownPanel = Boolean(countdownLabel && (stageCountdown || hasPersonalExtension));
  ...
  function renderVotingList() {
    ...
    return (
      <div className="grid gap-4">
        {showCountdownPanel ? (
          <section className={`rounded-[1.5rem] border px-4 py-3 ${countdownUrgent ? "border-rose-300/16 bg-rose-400/10" : "border-arenaBeam/16 bg-arenaBeam/10"}`}>
            <div className="flex flex-wrap items-center justify-between gap-3">
              <div className="min-w-0">
                <p className={`label-copy text-[11px] uppercase tracking-[0.28em] ${countdownUrgent ? "text-rose-100" : "text-arenaBeam"}`}>
                  {hasPersonalExtension ? text.countdownPersonal : countdownUrgent ? text.countdownWarning : text.countdownGlobal}
                </p>
                <p className="mt-1 text-xs leading-5 text-arenaMuted">
                  {hasPersonalExtension ? text.countdownPersonalText : text.countdownGlobalText}
                </p>
              </div>
              <span className={`show-chip px-3 py-2 text-sm ${countdownUrgent ? "text-rose-100" : "text-white"}`}>
                <Lock size={14} />
                {countdownLabel}
              </span>
            </div>
        ...
        <section className="show-card px-4 py-3 md:p-4">
          <div>
            <input
              className="arena-input h-11 pr-4 text-base md:h-12 md:text-sm"
          ...
          {qualificationCutoff ? (
            <div className="mt-3">
              <span className="inline-flex items-center gap-[0.45rem] rounded-full border border-emerald-300/14 bg-white/[0.05] px-[1.05rem] py-[0.55rem] text-[11px] uppercase tracking-[0.22em] text-emerald-100">
                <CheckCircle2 size={13} />
                {isPhone
                  ? (language === "ru"
          ...
          <SortableContext items={filteredActs.map((act) => act.code)} strategy={verticalListSortingStrategy}>
            <section className="grid gap-3">
              {filteredActs.flatMap((act) => {
                ...
                return [
                  isAfterCutoff ? (
                    <div key={`cutoff-${stageKey}-${act.code}`} className="show-panel-muted relative overflow-hidden border border-amber-200/10 px-3 py-2.5 md:px-3.5 md:py-3">
                      ...
                      <div className="relative flex items-center">
                        <span className="show-chip px-3 py-1.5 text-[10px] uppercase tracking-[0.2em] text-amber-100">
                          {language === "ru"
                            ? `С места #${qualificationCutoff + 1} — вне финала`
                            : `From #${qualificationCutoff + 1} — outside the final zone`}
                        </span>
                      </div>
                    </div>
                  ) : null,
        ...
        {selectedAct ? (
          <div className="grid gap-4 md:gap-5">
              ...
              {isPhone ? (
                <div className="mt-3 grid grid-cols-[minmax(0,1fr)_2.75rem_2.75rem] gap-2">
                  ...
                  <button
                    ...
                    disabled={stageClosed || rankingMap[selectedAct.code] === 1}
                  ...
                  <button
                    type="button"
                    className="arena-button-secondary inline-flex h-10 w-11 items-center justify-center rounded-[1rem] px-0 text-sm"
                    disabled={stageClosed || rankingMap[selectedAct.code] === ranking.length}
                    onClick={() => moveArtistBy(selectedAct.code, 1)}
            ...
            {selectedActLinks.videoUrl ? (
              <ArtistVideoPanel href={selectedActLinks.videoUrl} title={text.watchVideo} hint={text.watchVideoHint} />
            ) : null}

            <ArtistAboutPanel
              act={selectedAct}
              title={text.aboutArtist}
              facts={getActFacts(selectedAct)}
              fallbackBlurb={getActBlurb(selectedAct)}
            />
          </div>
        ) : null}
      </BottomSheet>
