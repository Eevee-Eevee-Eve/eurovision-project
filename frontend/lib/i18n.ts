...
const ROOM_META: Record<string, { tagline: Record<Language, string>; cityLabel: Record<Language, string> }> = {
  "neon-arena": {
    tagline: {
      ...
      en: "Eurovision at the Morozovs 2026",
    },
    cityLabel: {
...
const ENGLISH_DISPLAY_ALIASES: Record<string, string> = {
...
export function formatDisplayText(language: Language, value: string) {
  if (language !== "en") {
    return value;
  }

  const normalized = String(value || "").trim();
  return ENGLISH_DISPLAY_ALIASES[normalized] || transliterateCyrillic(value);
}
